...
- Visualização de informação para divulgação científica: uma metodologia da Professora
- Aplicativo de interação sensorial e visualização sinestésica de Filipe Dias de Oliveira e da Prof. Doris Kosminsky (da EBA-UFRJ)
- Visualização sobre dados de redes sociais e Visualização de dados: representações gráficas e padrões de dados de Carolina Riff, Amanda Pereira, Barbara Castro e da Prof. Doris Kosminsky (da EBA-UFRJ)
- Desenhe seu trajeto: visualização de dados sobre o uso do espaço e a participação de usuários no projeto de sinalização da Universidade Federal do Espírito Santo do Prof. Mauro Pinheiro (UFES), Ricardo Esteves Gomes e Matheus Rocha de Souza
- Visualização de dados: avanços por pesquisadores brasileiros por Rodrigo Medeiros (IFPB) e Julia Rabetti Giannella (doutoranda da ESDI)
